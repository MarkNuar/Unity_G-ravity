	...
	SubShader
	{
		...
		Pass
		{
			...
			float4 frag (v2f i) : SV_Target
			{
				...
				float dstToOcean = ray_sphere(planetCentre, oceanRadius, ray_pos, ray_dir).x;
				float dstToSurface = min(scene_depth, dstToOcean);
				
				float2 hitInfo = ray_sphere(planetCentre, atmosphereRadius, ray_pos, ray_dir);
				float dstToAtmosphere = hitInfo.x;
				float dstThroughAtmosphere = min(hitInfo.y, dstToSurface - dstToAtmosphere);
				
				if (dstThroughAtmosphere > 0) {
					const float epsilon = 0.0001;
					float3 pointInAtmosphere = ray_pos + ray_dir * (dstToAtmosphere + epsilon);
					float3 light = calculateLight(pointInAtmosphere, ray_dir, dstThroughAtmosphere - epsilon, originalCol, i.uv);
